...
export default function DoctorList() {
  ...
  const filteredList = doctors.filter((doctor) => {
    const isFilterActive =
      filters.specialty?.length || filters.rate || filters.trait?.length;
    ...
    if (Boolean(filters.gender?.length)) {
      const matchesGender = doctor.gender
        .trim()
        .toLowerCase()
        .includes(filters.gender!.trim().toLowerCase());
    ...
    if (filters.rate) {
      const matchesRate =
        filters.rate.min !== undefined && filters.rate.max === undefined
          ...
          : filters.rate.min === undefined && filters.rate.max !== undefined
            ? doctor.averageRating < filters.rate.max
            : true;
